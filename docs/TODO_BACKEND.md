...
### ⚠️ 9. Уровень автора в постах форума

Бейдж «Ур. N» на странице темы (`/forum/thread/:id`) рендерится только если
`post.author.level` присутствует в ответе — сейчас не проверено, отдаёт ли
`GET /forum/threads/:id/posts` это поле.

**Требуется**: добавить `level` пользователя в ответ `GET /forum/threads/:id/posts`.
