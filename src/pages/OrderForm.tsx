...
export default function OrderForm({ initial = {}, alreadyReserved = 0, showFiles = true, submitLabel, submittingLabel, pageTitle, onSubmit }: Props) {
  ...
  // исполнителю просто показывается цена ниже (см. reshbirga POST/PATCH /orders).
  const balance = parseFloat(String(profile?.balance ?? 0))
  const extraNeeded = Math.max(0, Math.round((amount - alreadyReserved) * 100) / 100)
  const insufficient = amount > 0 && balance < extraNeeded
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await onSubmit({
        ...
        base_amount: amount, category: category || undefined,
      }, files)
    } catch (err: any) {
  ...
  return (
    <div className="max-w-[720px] mx-auto">
      <div className="text-slate-200 text-[1.4rem] font-bold mb-6">{pageTitle}</div>
      <form onSubmit={handleSubmit}>
        ...
        {amount > 0 && (
          <div className={`rounded-lg py-[14px] px-4 mb-6 border ${insufficient ? 'bg-[#1f0808] border-[#ef4444]' : 'bg-[#0d2620] border-teal-legacy-hover'}`}>
            <div className="text-slate-500 text-[0.78rem] mb-1.5">{alreadyReserved > 0 ? 'Новая сумма (спишется или вернётся разница)' : 'К списанию с баланса'}</div>
