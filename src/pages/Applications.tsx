...
function StarsRow({ rating, count }: { rating?: number; count?: number }) {
  ...
  return (
    <div style={S.rating}>
      ...
      <span style={{ color: '#64748b', fontSize: '0.78rem' }}>({count || 0} отзывов)</span>
    </div>
  )
}
...
export default function Applications() {
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto' }}>
      ...
      {apps.length === 0 && (
        <div style={{ color: '#64748b', textAlign: 'center', padding: '3rem' }}>Заявок пока нет</div>
      )}
      ...
      {apps.map((app: any) => (
        <div key={app.id} style={{ ...S.card, opacity: app.status !== 'pending' ? 0.5 : 1 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
            ...
            {app.status === 'pending' && <button style={S.selectBtn} onClick={() => setModal(app)}>Выбрать</button>}
            {app.status === 'accepted' && <span style={{ color: '#22c55e', fontSize: '0.82rem', fontWeight: 600 }}>✓ Выбран</span>}
            {app.status === 'rejected' && <span style={{ color: '#64748b', fontSize: '0.82rem' }}>Отклонён</span>}
          </div>
        </div>
      ))}
        ...
        <div style={S.overlay} onClick={() => !acting && setModal(null)}>
          <div style={S.modal} onClick={e => e.stopPropagation()}>
            ...
            <div style={S.modalBtns}>
              ...
              <button style={S.okBtn} onClick={handleSelect} disabled={acting}>{acting ? 'Обработка...' : 'Подтвердить выбор'}</button>
            </div>
          </div>
        </div>
      )}
