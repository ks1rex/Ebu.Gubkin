...
// ponytail: ~110 строк вместо react-markdown — используется только то, что реально
// есть в наших текстах: заголовки, списки, таблицы, жирный, `код` и ссылки.
// Понадобится остальной markdown — тогда и ставить зависимость.
// Общий рендерер для гайда администратора (pages/Admin/Help.tsx) и юридических
// страниц (pages/Legal.tsx).

/** `**жирный**`, `` `код` `` и `[текст](ссылка)` внутри строки. Без вложенности. */
function inline(text: string, key: string) {
  const parts = text.split(/(\*\*[^*]+\*\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g).filter(Boolean)
  return parts.map((p, i) => {
    const k = `${key}-${i}`
    if (p.startsWith('**') && p.endsWith('**'))
      return <strong key={k} className="text-ink font-semibold">{p.slice(2, -2)}</strong>
    if (p.startsWith('`') && p.endsWith('`'))
      return (
        <code key={k} className="px-1 py-0.5 rounded bg-white/[.07] text-accent text-[0.9em] font-mono">
          {p.slice(1, -1)}
        </code>
      )
    const link = /^\[([^\]]+)\]\(([^)]+)\)$/.exec(p)
    if (link) {
      ...
      const cls = 'text-accent-muted hover:text-accent underline underline-offset-2 transition-colors'
      // Внутренние адреса — через Link, иначе react-router не видит перехода
      // и страница перезагружается целиком (а с basename ещё и мимо приложения).
      return href.startsWith('/')
        ? <Link key={k} to={href} className={cls}>{label}</Link>
...
export function renderMarkdown(md: string) {
  // HTML-комментарии — служебные пометки для разработчика, на странице их быть не должно
