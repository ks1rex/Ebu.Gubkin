...
**The admin guide is generated, not hand-maintained.** `src/pages/Admin/adminGuideText.ts` (rendered by `pages/Admin/Help.tsx`) is produced from `reshbirga/ADMIN_GUIDE.md` by `npm run sync-guide` — edit the `.md` in the reshbirga repo and re-run, never the `.ts`. It's a copy because Vite can't import outside its root. `Help.tsx` is a ~90-line markdown subset renderer (headings h1–h3, tables, lists, bold, inline code) — if a guide edit needs anything beyond that, the renderer has to grow first.
