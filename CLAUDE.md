...
**Reports export as CSV + a printed report document.** `src/lib/exportCsv.ts` handles CSV (`;`-separated, BOM-prefixed for Excel in a RU locale) plus `fetchAllPages` — every "export everything" path loops pages, because PostgREST silently truncates at 1000 rows no matter what `limit` says. `src/lib/printReport.ts` builds a standalone report document (header, filter parameters, full table, totals) in a hidden iframe and prints *that*, so the file no longer contains nav chrome or one page of a table; the `@media print` block in `index.css` only matters for ad-hoc printing of a live page now. Still no xlsx/jspdf dependency: Cyrillic in jsPDF means embedding a TTF (~300KB–1.5MB base64) and hand-paginating tables — add it only if a PDF must be produced without a human in the loop (e.g. server-side scheduled reports).
