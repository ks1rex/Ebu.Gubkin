...
- `AdminRoute` — same, but gates on `profile?.is_admin`, **and** on MFA: if the admin has a verified TOTP factor but the session is only `aal1`, it renders a code prompt instead of the children. This is the single client-side MFA gate for the whole admin tree — the reshbirga backend rejects every `/admin/*` call from an `aal1` session with `{ code: 'MFA_REQUIRED' }`, so waiting for that error per page would mean handling it in ~16 pages. Enrollment UI is `pages/Admin/TwoFactor.tsx` (Supabase's native `supabase.auth.mfa.*` — no custom secret storage, no QR library); the post-login step is in `pages/Login.tsx`.
...
**Reports export as CSV + browser print.** `src/lib/exportCsv.ts` is the whole story: `;`-separated, BOM-prefixed (Excel in a RU locale), and "PDF" is `window.print()` against the `@media print` block at the bottom of `index.css` (which hides nav/aside and flattens the glass cards). No xlsx/jspdf dependency — add one only if a report needs real PDF layout.
